...
async function getUserRole(userId, groupId) {
  const r = await axios.get(`https://groups.roblox.com/v1/users/${userId}/groups/roles`);
...
async function setRole(userId, roleId, groupId, apiKey) {
  ...
  const m = memberships.data.groupMemberships.find(x => x.user.userId === userId);
...
app.post("/api/auth/roblox", async (req, res) => {
  try {
    ...
    const robloxId = r.data.data[0]?.id;
    if (!robloxId) return res.status(404).json({ error: "Roblox user not found" });

    // check existing account
    let user = await db.users.findOne({ robloxId });
